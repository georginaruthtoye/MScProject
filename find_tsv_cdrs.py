...
# This script takes as input a folder containing zip archives (.zip) 
# downloaded from the iReceptor Public Archive. First, the data for analysis
# is copied into folder /d/projects/u/tg001/TSV/data ready to run the script.
#
# After determining filepaths of files held by the defined directory, this
# script unzips any ZIP archive (.zip) files. Once unzipped, each archive
# represents one sample data unit, and consists of three associated files:
#
# (1) The main TSV (.tsv) file, which contains the sample data unit itself
#     i.e. one set of pre-processed Ig-seq sequences;
...
# In addition to the sample ZIP archives, there was one additional ZIP
# archive containing general study metadata in the form of a TSV file,
# which was also processed and saved.
#
# The script can also be used to extract the same data from regular TSV
# files, and could be adapted for other databases using the .tsv format.
#
# The aim was to extract all read sequence_ids & associated CDR-H3 sequences
# from each sample. This was done by loading the data from each .tsv file
# into a PANDAS dataframe. Sample metadata was stored by saving the .json and
# .txt files directly to the server.
# 
# Once the script has run, data from each ZIP file will have been streamed
# into the directory /d/projects/u/tg001/TSV/programs as separate folders:
# 
# 1_archpaths:    Single file listing all filepaths to each archive
